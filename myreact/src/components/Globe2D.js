/* global am5, am5themes_Animated, am5map, am5geodata_worldLow */
...
;(() => {
  ...
  function initEmpty2DGlobe() {
    ...
    if (typeof window.am5 === "undefined") {
      console.error("AmCharts libraries not loaded. Make sure the script tags are included in your HTML.")
      return false
    }
  ...
  function initializeAmChart() {
    // Create root element
    root = window.am5.Root.new("amchart-map")
    ...
    var backgroundSeries = chart.series.push(window.am5map.MapPolygonSeries.new(root, {}))
    backgroundSeries.mapPolygons.template.setAll({
      fill: window.am5.color(0x0d0208),
      fillOpacity: 0.1,
      stroke: window.am5.color(0x00ff41),
      strokeOpacity: 0.2,
    })
    ...
    pointSeries = chart.series.push(window.am5map.MapPointSeries.new(root, {}))

    // Configure marker template
    pointSeries.bullets.push(() => {
      var circle = window.am5.Circle.new(root, {
        radius: 5,
        tooltipY: 0,
        fill: window.am5.color(0x00ff41),
        stroke: window.am5.color(0x0d0208),
        strokeWidth: 2,
      ...
      circle.states.create("hover", {
        radius: 7,
        fill: window.am5.color(0x64ffda),
      })
  ...
  function populate2DGlobe(ipData, ipPackets) {
    ...
    validIPs.forEach((ip) => {
      ...
      const dataItem = pointSeries.pushDataItem({
        ...
        fillColor: window.am5.color(threatColor),
      })
